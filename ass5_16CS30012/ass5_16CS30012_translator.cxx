...
/*Searches for argument string in table as argument and if the entry is not there in symbol table then creates an entry and returns pointer to that entry*/
...
/*Generates a temporary variable in current symbol table*/
SymNode* gentemp(SymTab* st){
	char str[20];
    sprintf(str,"t%d",noTemp++);
	return lookup(st,str);
}
...
//------First prints global symvol table , then table for each function and its contenst.
void printSymTab(){
	int i,j;
	SymTab sp;
...
void printquad(){
	...
	for(i=0;i<nextinstr;i++){
		...
		switch(qArray[i].op){
			...
			case(DIV):op="/";break;
			case(MULT):op="*";break;
			case(UMINUS):op="-";break;
			case(COPY):op="";break;
			case(PLUSPLUS):op="++";break;
			case(MINUSMINUS):op="--";break;
			case(NEG):op="~";break;
			case(NT):op="!";break;
			case(REM):op="%";break;
			...
			case(ISEQUAL):op="==";break;
			case(NOTEQUAL):op="!=";break;
			case(XOR):op="^";break;
			case(BITOR):op="|";break;
			case(BITAND):op="&";break;
			case(QUESTIONMARK):op="?";break;
			case(COLN):op=":";break;
			case(ASSIGNMENT):op="";break;
			case(LOGICALOR):op="||";break;
			case(LOGICALAND):op="&&";break;
			case(_GOTO):op="goto";break;
			case(FUNC):op="call";break;
			case(PARAM):op="param";break;
			case(_RETURN):op="return";break;
...
/*Checks the types of the 2 nodes and changes the type if required*/
...
/*Sets the result of the quads of all indices in the list equal to index passed as argument*/
...
//-----Converts int to bool for conditional statements where condition is not relational expression to set truelist and falselist
...
int main(){
	int i;
		...
		char temp[20];
		if(yyparse()==0) 
			printf("Congratulations! Program parsed successfully!\n");
		else 
			printf("Alas! Could not parse the Program!\n");
		printSymTab();
		printf("\n\n");
		printquad();
  	return 0;
